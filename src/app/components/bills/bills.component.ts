...
export class BillsComponent {
  ...
  filteredBills = signal<BillItem[]>(this.bills()
    .sort((a,b) => Number(a.status === 'Paid') - Number(b.status === 'Paid' ))
    .filter(b => {
      const billDate = new Date(b.dueDate);
      return billDate.getMonth() === this.selectedMonth().getMonth() && billDate.getFullYear() === this.selectedMonth().getFullYear();
    }));
  ...
  totalBillItems = computed(() => {
    if(this.selectedCategory() !== 'All'){
      return this.filteredBills()
      .filter(x => x.category === this.selectedCategory() && x.status !== 'Paid').length;
    }
    else{
      return this.filteredBills()
  ...
  filterBills(category: string) {
    ...
    if(category === 'All') {
      ...
      .filter(b => { 
        const billDate = new Date(b.dueDate);
        return billDate.getMonth() === this.selectedMonth().getMonth()
  ...
  getCategoryStyles(category: BillItem['category']) {
    switch (category) {
      ...
      case 'Utilities': return { bg: 'bg-sky-50', text: 'text-sky-600' };
      case 'Transportation': return { bg: 'bg-amber-50', text: 'text-amber-600' };
